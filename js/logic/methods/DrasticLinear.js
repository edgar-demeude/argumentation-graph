/**
 * @fileoverview Drastic Linear (Multiplicative with State Scaling) semantics.
 */

/**
 * Drastic Bipolar Linear Semantics.
 * Formula: σ(a) = min(1.0, (1 - Σ att) * (1 + Σ sup)) * stateMultiplier
 * 
 * Attacker at 1.0 kills target.
 * State is a linear filter (final scaling).
 *
 ...
 */
export function calculateDrasticLinear(nodes, categoryWeights, stateMultipliers, activeIds) {
  ...
  for (let iter = 0; iter < maxIterations; iter++) {
    ...
    nodes.forEach(n => {
      ...
      const argStrength = Math.min(1.0, Math.max(0, (1 - attackSum) * (1 + supportSum)));
      nextScores[n.id] = argStrength * nodeMultiplier;
